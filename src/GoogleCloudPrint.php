...
class GoogleCloudPrint {
	...
	function submit($document, $content_type, $printer_id, $title = null) {
		...
		$response = $this->httpClient->request ( 'POST', self::APIBASE . '/submit', [ 
				'form_params' => [ 
						'title' => $title,
	...
	/**
	 * Simple utility function for accepting invitations to a printer.
	 * Useful
	...
	function acceptInvitation($printer_id) {
		...
		$response = $this->httpClient->request ( 'POST', self::APIBASE . '/processinvite', [ 
				'form_params' => [ 
						'printerid' => $printer_id,
						'accept' => 'true' 
				] 
